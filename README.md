...
以下のようにプロジェクトのホーム画面が表示されたら、プロジェクトのアクセス完了です。</br>
![image](./images/3.png)

## 2. 資産の作成

**アセットタブ**に切り替えて、**新規資産**をクリックします。
画像の例では「Hands-on」としています。</br>
![image](./images/4.png)

新規資産画面が表示されたら、モデルの処理に表示されている**ビジュアル・フローとしてのモデル作成**（SPSS Modeler）をクリックします。</br>
![image](./images/5.png)
...
|ファイル名|内容|
|:--|:--|
|顧客属性_実績.csv|お客様の年齢や年収など属性情報|
|利用履歴_実績.csv|クレジットカードの履歴|
|反応_実績.csv|過去キャンペーンに反応したか|

|ファイル名|内容|
|:--|:--|
|顧客属性_予測.csv|お客様の年齢や年収など属性情報|
|利用履歴_予測.csv|クレジットカードの履歴|
...
ノードにカーソルを合わせて、**データのプレビュー**を選択してみてください。</br>
![image](./images/14.png)
...
利用履歴データを顧客毎・用途毎に「使った（T）/ 使っていない（F）」のフラグに変換し、縦持ちのデータを横持ちに簡単に変換します。</br>
![image](./images/003.png)

利用履歴_実績.csvのデータ型を読み込みます。
...
セット型フィールドはフラグ化したい列名である**用途**を選択し、**値の追加**をクリックします。</br>
![image](./images/20.png)

すべてフラグ化したいので、すべてのチェックボックスを**ON**にして**OK**をクリックします。</br>
![image](./images/22.png)

顧客番号で用途のフラグをまとめたいので、集計キーを**ON**にして**列の追加**をクリックします。</br>
![image](./images/21.png)
...
エンコーディングが意図した通りにできているか確認をします。
出力パレットから、**表**ノードをフロー・キャンバスにドラッグ＆ドロップし、**フラグ設定→表**と接続します。</br>
![image](./images/25.png)

表ノードにカーソルを合わせて、**実行**をクリックします。
...
意図通り顧客番号毎にどの用途で利用したかを横持ちのテーブルに変換できました。</br>
![image](./images/27.png)
...
レコード操作パレットから、**マージ**ノードをフロー・キャンバスにドラッグ＆ドロップし、**フラグ設定→マージ**、**顧客属性_実績.csv→マージ**、**反応_実績.csv→マージ**と接続します。</br>
![image](./images/28.png)
...
グラフからデータの抽出範囲を検討し、今回のキャンペーンのターゲットを20〜60歳・年収200万以上に設定します。</br>
![image](./images/61.png)

レコード操作パレットから、**条件抽出**ノードを年齢ノードと表ノードの間にドラッグ＆ドロップし、**年齢→条件抽出**と接続します。</br>
![image](./images/63.png)
...
ツリー図を見てみると、「航空会社を使い、40歳以下34歳以上で、百貨店を使う顧客のキャンペーンの反応確度は52％である」ことがわかります。
ツリー図はこのようなルールの集まりになります。</br>
![image](./images/35.png)
...
出力パレットから、**解析**ノードをフロー・キャンバスにドラッグ＆ドロップし、**キャンペーン反応→解析**と接続します。</br>
![image](./images/78.png)

**解析**ノードを**実行**し、出力結果を表示します。
学習・テストの両方に対して、正答率を表示できます。</br>
![image](./images/80.png)
...
**選択**ノードをダブルクリックして、右上の鉛筆マークをクリックし`年齢20歳から60歳、年収200万以上で条件抽出`と入力したら、**保存**をクリックします。</br>
![image](./images/81.png)
